...
let nh = "[t*[<[2|4|8|16]>/3]]"
d1:stack(
s("ncircuit:6").loopAt(1).randslice(8).fast("[16]*32").degradeBy(0.7).clip(irand(2).add(1)).clip(1).cut(1)
.slowf(2,x=>x.repeatCycles(4))
.gain(0.3).lpf(3000).lpq(6)
,
stack(
s("nk:23").struct(nk).note(29).clip(4).gain(1).cut(1).coarse(3)
.distort(1).nmod(nenv(1).natt(0.25).nsus(1).nsync(1).ntype('exp')).distortvol(0.25).distorttype('sinefold')
,
s("ns:26").struct(ns).clip(irand(6).div(2)).gain(0.7).cut(1).lpf(perlin.slow(1.5).range(900,9000)).lpq(3),
).coarse(irand(6))
