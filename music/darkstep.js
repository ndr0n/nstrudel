...
d1: stack(
  stack(
    ...
    .s("nk:27").gain(0.5).clip(3).cut(1).dist("2:0.5:sinefold")
    .nbpf(perlin.slows(3,487).rangex(1,10000),1,perlin.slows(3,512).range(1,10000))
    ,
    "[~ ~ ~ ~ t ~ ~ ~]*2".s("ns:3").gain(0.5).clip(1).cut(1).dist("2:0.5:hard")
  ...
  // ,
  s("[nneuro:9]").loopAt(4).slice(12, "[0|1|2|3|4|5|6|7]*4?").gain(1).clip("[1|2]*32").cut(5)
  ...
  // "[~ ~ t ~]*4".s("nfx:9").legato(nrand(0.25,0.5,739)).gain(0.5).nbpf(perlin.slows(6,326).rangex(1,10000),3,perlin.slows(3,956).range(1,10000)),
).orbit(1)
