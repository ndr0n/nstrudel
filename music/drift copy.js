...
d1: stack(
  ...
  note(nacid).trans(30).s("saw").gain(0.7).clip(3).cut(6).lpf(nrand(30,300,548)).lpe(nrand(-3,6,478)).lpd(nrand(0,0.375,046)).lps(nrand(0,0.375,875)).lpq(nrand(12,24,754))
  .sometimesBy(0.25,x=>x.distort("2:0.25:sinefold")).early(8)
  .sometimesBy(0.25, x=>x.gain(nlfo("sine","[n30.125|n37.0625]",1,1))).early(16)
  .nbpf(perlin.slow(3).range(300,3000).early(586),3,6000),
).orbit(1).dat(0.5)
.delay(0.25).delayt(0.0325).delayfb(nrand(0,0.975,974))
.sometimesBy(0.25,x=>x.scramble(16))
// .cps(nrand(0.25,0.75,125))
...
d2: stack(
  ...
  s("ndrone:2").fast(9).degradeBy(0).note("[28|35|40]").clip(1).begin(perlin.slow(3).early(361).range(0,0.33)).gain(0.75).nbpf(nrand(600,6000,126),3,6000)
  // note(nchords).trans(30).s("piano").clip(1).gain(0.7).nbpf(nrand(300,3000,746),3,6000),
...
// solid(0,0,0)
// .layer(src(s0),1)
// .color(1,H(nacid.div(9)),H(ns.add(1).mul(nrand(1,3,167))))
