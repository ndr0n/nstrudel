...
,
stack(
s("namen:23").loopAt(1).randslice(8).fast(8).degradeBy(0/9).gain(1/4).clip(wchoose([1,1])).cut(2)
.coarse(irand(0)).ndjf(perlin.slow(3).late(583).range(0.1,0.5)).ndjfq(0.1)
,
// s("namen:26").loopAt("[1|2]*16").randslice(16).fast(8).degradeBy(1/9).gain(1/6).clip(wchoose([1,1],[0.25,0])).cut(2)
// .coarse(irand(0)).ndjf(perlin.slow(3).late(583).range(0.2,0.4)).ndjfq(0.15)
// .slowf(2,x=>x.repeatCycles(3))
// ,
// s("namen:3").loopAt("[1|2]").randslice(16).fast(16).degradeBy(1/3).late(238).gain(1/3).clip(wchoose([1,1],[0.25,0.5])).cut(2)
// .ndjf(perlin.slow(3).late(583).range(0.2,0.4)).ndjfq(0.1)
// ,
stack(
...
,
s("ns:4").struct(ns).clip("[0.5|0.75|1|1.5|2]*32").gain(0.5).cut(1)
.coarse(irand(9))
