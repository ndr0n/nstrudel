...
d1: stack(
 stack(
    ...
    "[t t t t]*2".s("hh:1").gain(1).clip(0.125),
...
d2: stack(
  // note("[0 3 5 7 12 15 19 24 27 31 36]*1").trans(28).s("saw").clip(1).lpf(800).gain(1),
  s("supersaw").freq("40,120").detune(1/8).gain(2)
  // .lpf(100).lpq(1)
  // .dist("2:0.25:sinefold")
  .gain(nmod("lfo:sine:39.675:1:0"))
  .gain(nmod("lfo:sine:119.875:1:0"))
  .lpf(nmod("lfo:sine:39.5:2000:2020")).lpq(1)
  .hpf(nmod("lfo:sine:119.75:2000:2020")).hpq(1)
  .gain(2)
  // .coarse(irand(6))
  // .crush(nmod("lfo:sine:79.9:8:12"))
  // .lpf(1000).lpq(1)
)  
.room(0.375).size(9)
.delay(0.25).delayt(0.33).delayfb(0.5)

all(x => x.compressor("-8:8:8"))
