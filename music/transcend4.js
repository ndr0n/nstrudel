...
d1:stack(
stack(
// s("supersaw").slow(irand(9)).detune(1/9).note(31).trans(0).trans("[0,7]").gain(1).clip(1).cut(22)
// .nfm(nlfo(100).naddnote(0).naddfreq(rand.slow(4).range(-1/3,1/3)))
// .ndjf(0.1).nmod(nenv(0.2).ndec(1).nsync(1).ntype('exp')).ndjfq(0.125)
// ,
).orbit(2)
...
,
stack(
// s("namen:1").loopAt(0.5).randslice(8).fast(8).degradeBy(1/3).gain(1/3).clip("[0.25|1]*32").cut(2)
// .coarse(irand(9))
// .ndjf(perlin.slow(3).late(583).range(0.3,0.5)).ndjfq(0.15)
// ,
// s("namen:4").loopAt(2).randslice(8).fast(8).degradeBy(0).gain(1/3).clip(wchoose([1,1],[0.25,0.25])).cut(1)
// .coarse(irand(6)).ndjf(perlin.slow(3).late(583).range(0.3,0.5)).ndjfq(0.15)
// ,
s("namen:6").loopAt(2).randslice(16).fast(16).degradeBy(0.25).gain(1/3).clip(wchoose([1,1],[0.25,0.25])).cut(2)
