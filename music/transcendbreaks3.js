...
d1:stack(
stack(
s("supersaw").slow(irand(9).add(3)).detune(1/9).note(24).trans(0).trans("[0]").gain(0.5).clip(1).cut(20)
.nfm(nlfo(100).naddnote(19).naddfreq(rand.slow(4).range(-1/3,1/3)))
...
stack(
stack(
s("namen:3").loopAt("[1|2|2]").randslice(16).fast(8).degradeBy(1/9).late(238).gain(1/3).clip("[1|0.5|1|0.25|1]*32").cut(2)
...
,
s("ns:28").struct(ns).clip("[0.75|1|1.5]*32").gain(0.5).cut(1)
.coarse(irand(9))
