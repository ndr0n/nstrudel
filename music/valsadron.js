...
d1: stack(
  stack(
    ...
    s("saw").struct(nk).clip(8).cut(2).gain(2).note(25).lpf(nadsr("exp",1,0,0,0,3000,30,1)).lpq(nrand(3,9,258)),
    s("ns:3").struct(ns).clip(irand(8).add(1).div(2)).cut(1).gain(0.5).nbpf(nrand(300,6000,126),3,6000),
  ).coarse(irand(6)),
  s("nhh:0").struct(nh).clip(nrand(0.25,0.25,363)).gain(0.5),
...
// .sometimesBy(1, x=>x.ncps(nrand(0.125,0.75,125))).early(24)
// .slowf(2, x=>x.repeatCycles(4))
// .spectrum({speed: 1})

d2: stack(
  // note(nchords).trans(30).clip(1).s("piano").gain(1).lpf(6000).lpq(9),
