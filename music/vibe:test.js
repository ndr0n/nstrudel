await initHydra({feedStrudel: 1, detectAudio: true});
const synths = (await import('https://cdn.jsdelivr.net/gh/ndr0n/nstrudel/synths/functions.js')).load();
register('randslice', (val, pat) => pat.slice(val, "[0]".add(irand(val))) );
samples('github:ndr0n/nsamples')

webdrone(true)
setcps(0.5)

d2:stack(
// s("nchords:12").loopAt(4).randslice(16).fast(16).note(24).late(48).degradeBy(0.7).clip(irand(2).add(1)).cut(1)
s("nchords:12").note(24).loopAt(4).randslice(16).fast(16).late(48).degradeBy(0.7).clip(irand(2).add(1)).cut(1)
.gain(0.5).lpf(6000).lpq(9)
// .slowf(2,x=>x.repeatCycles(4))
,
).orbit(2).scope({pos: 0, scale: 0.25})
.room(1/9).size(6)
.delay(1/9).delayt(1/3).delayfb(rand.range(0,1/6)).delaysync(1)

let nk = "[<t ~> ~ t?0.7 <~ t> ~ ~ ~ ~]*1?0.25"
let ns = "[~ ~ ~ ~ t ~ ~ ~]*1?0.25"
let nh = "[t*[<[2|4|8|16]>/3]]"
d1:stack(
// s("ncircuit:0").loopAt(1).randslice(16).fast(16).late(16).degradeBy(0.7).clip(irand(2).add(1)).cut(1)
// .gain(1/3).lpf(3000).lpq(6)
// .slowf(2,x=>x.repeatCycles(4))
// ,
stack(
// s("namen:12").loopAt(4).randslice(32).fast(16).degradeBy(0.7).late(32).clip(irand(2).add(1)).cut(1).gain(0.2),
s("nk:23").struct(nk).note(29).clip(4).gain(1).cut(1)
.distort(1).nmod(nenv(1).natt(0.25).nsus(1).nsync(1).ntype('exp')).distortvol(0.25).distorttype('sinefold')
.lpf(perlin.slow(1).range(900,9000)).lpq(9)
,
s("ns:28").struct(ns).clip(2).gain(0.4).cut(1)
.lpf(perlin.slow(2).range(900,9000)).lpq(3)
,
).coarse(irand(6))
.sometimesBy(1/6,x=>x.scramble(16))
,
s("nhh:0").struct(nh).clip(0.125).gain(0.25).cut(4).lpf(perlin.slow(3).range(300,9000)).lpq(3),
).orbit(1).scope({pos: 0, scale: 0.5})
.delay(1/3).delayt("1".div(perlin.slow(3).range(70,80))).delayfb(rand.range(0,8/9))
// .sometimesBy(1/1,x=>x.cps(rand.fast(5).range(0.25,0.625)))
// .slowf(2,x=>x.repeatCycles(4))
  
solid(0,0,0).layer(src(s0),1)
.kaleid(2)
.out()